...
async function ReadOffers(req, res) {
    let claim;
    const arr=[];
      const data = await dataHandling.Read("Offers", req.body.DocId, req.body.index, req.body.Keyword, req.body.limit, ["CountryId","==", req.body.CountryId,"Active","==",true,"CoupensCount",">",0]);
  ...
  async function BuyOffer(req, res) {
     try {
      const Userdata = await dataHandling.Read("Users", req.body.UserId);
      const Coupen = db.collection("Offers").doc(req.body.OfferId).collection("Coupens").limit(1).get();
  await db.collection("Users").doc(req.body.UserId).collection("Rewards").doc(Coupen.id).set({...Coupen.data()},{"merge":true})
  await dataHandling.Delete(`Offers/${req.body.OfferId}/Coupons`,Coupen.id)
